  ...
  describe "hash extensions" do
    ...
    describe "#digging" do
      it "gets values" do
        ...
        expect(dataset.digging "author", "name").must_equal "Isaac Asimov"
        expect(dataset.digging "author", "birhtdate").must_equal "1920-01-02"

        expected = dataset["series"].map{ |serie| serie["name"] }
        expect(dataset.digging "series", '&', "name").must_equal expected

        dataset.dig("series", 0, "books").each_with_index do |book, index|
          expect(dataset.digging "series", 0, "books", index, "title").must_equal book["title"]
        end

        expected = dataset.dig("series", 0, "books").map{ |book| book["title"] }

        expect(dataset.digging "series", 0, "books", '&', "title").must_equal expected

        expected = dataset["series"].map do |serie|
          serie["books"].map{ |book| book["title"] }
        end

        expect(dataset.digging "series", '&', "books", '&', "title").must_equal expected
        ...
        expect{ dataset.digging "series", '&', "books", '&', "book", "title" }.must_raise StandardError

        expect(dataset.digging "series", "books", '&', "book", "title").must_equal expected
      end
    end
  end
